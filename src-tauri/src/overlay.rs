...
use crate::constants::{
    INDICATOR_MARGIN, INDICATOR_MARGIN_BOTTOM, INDICATOR_WINDOW_PADDING,
    LIVE_METER_ANALYSIS_SAMPLES, LIVE_METER_BAR_COUNT, LIVE_METER_FULL_PEAK, LIVE_METER_FULL_RMS,
    LIVE_METER_SILENCE_PEAK_THRESHOLD, LIVE_METER_SILENCE_RMS_THRESHOLD,
};
use crate::state::{
...
const SHELL_PADDING_HEIGHT: i32 = 24;
const COMPACT_INDICATOR_HEIGHT: i32 = 46;
const PILL_PADDING_WITH_TRANSCRIPT: i32 = 30;
const PILL_PADDING_WITHOUT_TRANSCRIPT: i32 = 24;
...
fn fallback_indicator_window_size_impl(settings: &Settings) -> (i32, i32) {
...
pub(crate) fn fallback_indicator_window_size(settings: &Settings) -> (i32, i32) {
    fallback_indicator_window_size_impl(settings)
}
...
fn indicator_origin(
    ...
    let y = match position {
        ...
        | OverlayPosition::Caret => top + height - indicator_height - INDICATOR_MARGIN_BOTTOM,
    };

    Some(PhysicalPosition::new(
        x.clamp(left, work_max_x.max(left)),
        y.max(top),
    ))
}
...
pub(crate) fn update_indicator_window(app: &AppHandle, shared: &SharedState) {
    ...
    let (indicator_width, indicator_height) = measured_size.unwrap_or_else(|| {
        fallback_indicator_window_size(&settings)
    });
...
#[cfg(test)]
mod tests {
    use super::{fallback_indicator_window_size, measure_overlay_levels};
    use crate::state::{OverlayPosition, Settings};
    ...
    #[test]
    fn preserves_transcript_width_presets() {
