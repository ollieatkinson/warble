...
impl ParakeetTdt {
    pub fn load(model_root: &Path) -> Result<Self> {
        ...
        let config: ModelConfig = serde_json::from_slice(
            &fs::read(model_dir.join("config.json")).context("failed to read model config")?,
        )
        ...
        let preprocessor = match load_session(&model_dir.join("nemo128.onnx")) {
            Ok(session) => Some(session),
            Err(error) => {
                eprintln!("preprocessor session unavailable, using Rust extractor: {error}");
                None
            }
        };
        ...
        Ok(Self {
            preprocessor,
            encoder: load_session(&model_dir.join("encoder-model.int8.onnx"))
                .context("failed to load encoder model")?,
            decoder: load_session(&model_dir.join("decoder_joint-model.int8.onnx"))
    ...
    let mono = match (spec.sample_format, spec.bits_per_sample) {
        (hound::SampleFormat::Int, 16) => {
            ...
            samples
                .chunks(channels)
                .map(|frame| {
                    let sum = frame.iter().map(|sample| *sample as f32 / i16::MAX as f32).sum::<f32>();
                    sum / channels as f32
                })
                .collect()
        }
...
pub fn resample_to_16khz(input: &[f32], from_rate: u32) -> Vec<f32> {
    ...
    let output_len = ((input.len() as f64) * SAMPLE_RATE as f64 / from_rate as f64).round() as usize;
...
fn build_centered_hann_window() -> Vec<f32> {
    ...
    for index in 0..WIN_LENGTH {
        window[pad + index] =
            0.5 - 0.5 * ((2.0 * std::f32::consts::PI * index as f32) / (WIN_LENGTH - 1) as f32).cos();
    }
    window
}
...
fn build_mel_filter_bank(num_mels: usize) -> Vec<Vec<f32>> {
    ...
    for (index, value) in mel_points.iter().enumerate() {
        bins[index] = (((N_FFT + 1) as f32 * mel_to_hz(*value)) / SAMPLE_RATE as f32).floor() as usize;
    }
